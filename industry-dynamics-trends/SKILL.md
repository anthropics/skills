...
- **交付物**：一次分析工作流产出的 **MD 报告**（按模板填写，含总览、动态至少 10～N、总结与展望、来源链接索引）；报告可配备**图文/视频证据**，采集可根据链接调用浏览器@browser 、分析可 handoff 多模态模型（见 `references/multimedia-curation.md`、`checklists/asset-collection.md`）。
...
2. **检索与核查**：严格执行 `references/verification-curation.md`（时间戳、信源交叉验证、挤水分、硬核过滤、原声引用、关键数据二次核查、高价值/噪音分级）；按需执行 `references/source-discovery.md` 中的信源维度，并遵守 `references/multimedia-curation.md` 对图片/视频的采集与引用要求。**多媒体采集**：可根据已找到的链接**调用浏览器**（如 MCP cursor-ide-browser 或其它 web browser 能力）打开目标页面，进行截图或下载/引用页面内图片；**图片与视频的内容分析**（界面解读、关键帧、演讲结论提取）应 **handoff 给具备多模态能力的模型**执行，并在报告中注明。行业实例内若有额外检索与筛选要求一并执行；配图与采集清单见 `checklists/asset-collection.md`。
