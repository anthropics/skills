...
- **Icons:** render `react-icons` to SVG (`ReactDOMServer.renderToStaticMarkup`), rasterize with `sharp` at ≥256px, and insert via `addImage({ data: "image/png;base64," + buf.toString("base64") })` — the `image/png;base64,` prefix is required. If `require('react-dom/server')` or `renderToStaticMarkup()` fails with an **incompatible React versions** error, do not spend tool calls trying to repair the sandbox with `npm install`: fall back immediately to a ReactDOM-free visual such as a PptxGenJS shape treatment or a hand-authored SVG string, then rasterize/insert it the same way. (`react-icons`, `react`, `react-dom`, and `sharp` are preinstalled — install only when the relevant `require` itself is missing.)
